...
@Component({ selector: 'pv-dashboard', imports: [NgIf, NgFor, CurrencyPipe, RouterLink], template: `
  <section class="heading"><div><span class="eyebrow">Private workspace</span><h1>Welcome back.</h1><p>Keep your catalogue accurate, organised, and ready to share.</p></div><div class="actions"><button *ngIf="dashboard && dashboard.productCount === 0 && dashboard.totalCategoryCount === 0" class="button secondary" [disabled]="seeding" (click)="loadDemoData()">{{ seeding ? 'Loading demo…' : 'Load demo data' }}</button><a class="button secondary" routerLink="/categories">Add category</a><a class="button" routerLink="/products">Add product</a></div></section>
...
export class DashboardComponent implements OnInit {
  ...
  loadDemoData(): void {
    ...
    this.api.seedDemoData().subscribe({
      next: () => { this.seeding = false; this.loadDashboard(); },
      error: response => { this.seeding = false; this.error = response.error?.message ?? 'Demo data could not be loaded.'; this.changeDetector.detectChanges(); }
    });
  }
}

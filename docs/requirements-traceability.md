...
| Registration and login | `AuthController`, Identity, one-time email verification codes, password reset, short-lived JWT bearer configuration, rotating refresh sessions | Register, verify by code, sign in, restore a session safely after reload, sign out, and recover a password through Angular. |
...
| Product view/add/edit/delete | Angular products screen, in-app delete confirmation, and `ProductsApiController` | Complete the product acceptance tests. |
...
| Auditing | `AuditableEntity`, `AuditTrailService`, `AuditEvents`, and `InventoryMovements` | Inspect created/updated records and history in MySQL. |
| Unit tests | `backend/tests/ProductVault.Tests` | Run `dotnet test ProductVault.sln`. |
| ERD, setup, technical docs | `README.md`, `docs/`, and SVG diagrams | Review [documentation index](index.md). |
